...
- For cloud event-sleeve allocation rounds, if lower alias allocations fail to beat the canonical control in both hostile and current windows while also diluting the positive-window uplift relative to the full-size alias, stop sleeve-size micro-tuning and keep the branch classified as a `positive-window shadow sleeve`.
